...
export interface BuiltServiceHeroPrompt {
  ...
  alt: string;
  /**
   * Punchy 3-6 keyword slug for the filename, generated by the LLM.
   * Keyword-dense (no filler words), brand+content aware, suitable for
   * Google Images SEO indexing on the filename. Always lowercase + hyphens.
   * Example: "pittsburgh-pre-war-kitchen-design-build"
   */
  seo_slug: string;
...
3. SEO_SLUG — a punchy 3-6 keyword slug for the image filename. Google Images indexes filenames; long alt-text-derived filenames truncate poorly and waste SEO budget on filler words ("a", "the", "by"). This slug is keyword-dense, brand+content aware, and shorter than the alt. Used as the filename when uploading to R2. Rules:
   - LOWERCASE only, words separated by hyphens
   - 3-6 keywords, ~30-50 characters total
   - NO filler words (a, the, by, of, in, with, and, etc.)
   - Include brand+location+content keywords that matter for image search (e.g., "pittsburgh-pre-war-kitchen-design-build" for a B2 kitchen hero)
   - Do NOT include "hero" or "service" generic words — those have no SEO value
   - Do NOT match the alt text shape — alt is full sentence, slug is punchy keywords
   - Will be appended with a random 4-char hash and .png/.jpg at upload time

All three outputs (image_prompt, alt, seo_slug) draw from the SAME brand catalog inputs to ensure visual + textual coherence with the rest of the brand's surfaces.
...
Schema:
{
  ...
  "seo_slug": "pittsburgh-pre-war-kitchen-design-build"
}`;
...
export async function buildServiceHeroPrompt(
  ...
  let parsed: { image_prompt?: string; alt?: string; seo_slug?: string };
  try {
  ...
  // Defensive normalization on seo_slug: enforce lowercase + hyphens + no
  // double-hyphens even if the model misbehaves slightly. Fall back to a
  // dash-joined service-name slug if the model omits it entirely.
  const rawSlug = (parsed.seo_slug ?? "").trim();
