...
function WebsiteSpinner({ siteId, siteUrl }: { siteId: string; siteUrl: string | null }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Domain management */}
      <div>
        <p className="mb-2 text-xs font-medium">Root Domain</p>
        {domainResult?.success && domainResult.domain ? (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono">{domainResult.domain}</span>
              <span className={`rounded-full px-2 py-0.5 text-[9px] font-medium ${
                domainResult.verified ? "bg-success/20 text-success" : "bg-warning/20 text-warning"
              }`}>
                {domainResult.verified ? "Active" : "Pending DNS"}
              </span>
              <button
                onClick={async () => {
                  setVerifying(true);
                  try {
                    const res = await fetch("/api/admin/website", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ site_id: siteId, action: "verify-domain", domain: domainResult.domain }),
                    });
                    const data = await res.json();
                    setDomainResult((prev) => prev ? { ...prev, verified: data.verified && data.configured } : prev);
                  } catch { /* ignore */ }
                  setVerifying(false);
                }}
                disabled={verifying}
            ...
            {domainResult.dnsRecords && domainResult.dnsRecords.length > 0 && (
              <div className="rounded border border-border bg-background p-3">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-[10px] font-medium">DNS Records</span>
                  <button
                    onClick={() => {
                      const text = (domainResult.dnsRecords || []).map(r => `${r.type}\t${r.name}\t${r.value}`).join("\n");
                      copyToClipboard(text, "all");
                    }}
                    className="text-[10px] text-accent hover:underline"
                  >
                  ...
                  <tbody>
                    {domainResult.dnsRecords.map((r, i) => (
                      <tr key={i} className="border-b border-border last:border-0">
                        ...
                        <td className="py-1.5">
                          <button
                            onClick={() => copyToClipboard(r.value, `wd-${i}`)}
                            className="text-[9px] text-muted hover:text-accent"
                          >
                            {copied === `wd-${i}` ? "!" : "copy"}
                          </button>
                ...
                <p className="mt-2 text-[9px] text-muted">
                  Root domain uses an A record (not CNAME). If on Cloudflare, set to DNS only.
                </p>
              </div>
            )}
          </div>
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch("/api/admin/website", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ site_id: siteId, action: "add-domain", domain: domainInput }),
                  });
                  const data = await res.json();
                  setDomainResult(data);
                } catch {
                  setDomainResult({ error: "Request failed" });
                } finally {
                  setAddingDomain(false);
                }
              }}
              disabled={addingDomain || !domainInput}
              className="bg-accent px-3 py-1 text-[10px] font-medium text-white hover:bg-accent-hover disabled:opacity-50"
            >
